...
class Scroll {
    ...
    getSeccionesArray() {
        let seccionesArray = Object.keys(this.seccionesJson).map((Seccion) => {
    ...
    removeClassNameArray(obj) {
        this.classActiveArray.map((className) => {
    ...
    isHover() {
        this.seccionesArray.map(({ id }) => {
            let section = document.getElementById(id);
            let sectionNav = document.getElementById(`Nav-${id}`);

            if (section && sectionNav) {
                let positionSectionTop = section.offsetTop;
                let sizeSection = section.offsetHeight;
                if (
    ...
    colorNavBar() {
        ...
        if (this.primeraSeccion && this.navBar) {
            ...
            var positionSection = this.primeraSeccion.scrollHeight - 20; // el -20 es para ajustar un poco.
